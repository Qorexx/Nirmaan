...
export const ArchitectureShowcasePage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen py-10 px-4 sm:px-8 bg-[#F7F5F0] text-slate-100">
      <div className="max-w-[1700px] mx-auto space-y-8">
        
        {/* Top Title Bar */}
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 pb-6 border-b border-[#D6D0C4]">
          <div className="space-y-2">
            ...
            <p className="text-sm sm:text-base text-[#44403C] font-mono max-w-4xl">
        ...
        <div className="flex items-center justify-start sm:justify-center overflow-x-auto no-scrollbar py-2">
          <div className="bg-[#EAE5DC] p-2 rounded-2xl border border-[#D6D0C4] flex gap-2 shadow-2xl">
        ...
        <div className="transition-all duration-300">
          ...
          {activeTab === 'architecture' && (
            <div className="space-y-8 animate-fadeIn">
              <div className="bg-[#F7F5F0] border border-cyan-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
                <h3 className="text-2xl font-extrabold text-[#1C1917] mb-4 flex items-center gap-3">
                  <ShieldCheck className="w-8 h-8 text-cyan-400" />
                  <span>The Autonomous Infrastructure Operating System Topology</span>
                </h3>
                <p className="text-[#44403C] text-base font-mono max-w-4xl mb-8 leading-relaxed">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-[#EAE5DC] p-6 rounded-2xl border border-blue-500/30 space-y-4">
                    <div className="w-12 h-12 rounded-xl bg-blue-500/20 text-blue-400 flex items-center justify-center font-bold text-xl">🏛</div>
                    <h4 className="text-xl font-bold text-[#1C1917]">Government Command Suite</h4>
                  ...
                  <div className="bg-[#EAE5DC] p-6 rounded-2xl border border-purple-500/30 space-y-4">
                    ...
                    <h4 className="text-xl font-bold text-[#1C1917]">AI Escrow Core & x402</h4>
                    <p className="text-sm font-mono text-[#44403C]">
                  ...
                  <div className="bg-[#EAE5DC] p-6 rounded-2xl border border-emerald-500/30 space-y-4">
                    ...
                    <h4 className="text-xl font-bold text-[#1C1917]">Blockchain Trust Layer</h4>
                    <p className="text-sm font-mono text-[#44403C]">
                ...
                <div className="mt-8 p-6 rounded-2xl bg-gradient-to-r from-blue-900/30 to-cyan-900/30 border border-cyan-400/30 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div>
                    <h5 className="font-extrabold text-[#1C1917] text-lg">Want to see this architecture live in action?</h5>
          ...
          {activeTab === 'workflow' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="bg-[#F7F5F0] border border-[#D6D0C4] rounded-3xl p-6 sm:p-8 shadow-2xl space-y-4">
                <div className="flex items-center justify-between pb-4 border-b border-[#D6D0C4]">
                ...
                <div className="font-mono text-xs text-[#44403C] space-y-2.5 max-h-72 overflow-y-auto pr-2">
                  {logs.slice().reverse().map((log, i) => (
                    <div key={log.id || i} className="flex items-start gap-3 p-2.5 rounded-lg bg-[#F7F5F0] border border-[#D6D0C4] hover:border-cyan-500/30 transition-colors">
                      <span className="text-[#78716C] font-bold shrink-0">[{log.timestamp}]</span>
                      <span className="text-cyan-400 font-bold uppercase shrink-0">[{log.stage}]</span>
                      <span className="text-[#44403C] flex-1">{log.message}</span>
                      {log.confidence && (
          ...
          {activeTab === 'api' && (
            <div className="space-y-6 animate-fadeIn">
              <div className="bg-[#F7F5F0] border border-amber-500/40 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-[#D6D0C4]">
                  <div>
                    <h3 className="text-xl font-extrabold text-[#1C1917] flex items-center gap-2.5">
                      <Zap className="w-6 h-6 text-amber-400 fill-current" />
                      <span>x402 Micropayment & Autonomous API Challenge Logs</span>
                    </h3>
                    <p className="text-xs font-mono text-[#44403C] mt-1">
                ...
                <div className="space-y-4">
                  {mockApiLogs.map((req, index) => (
                    <div key={req.id} className="bg-[#EAE5DC] border border-[#D6D0C4] rounded-2xl p-5 hover:border-amber-400/50 transition-all space-y-3 font-mono">
                      <div className="flex flex-wrap items-center justify-between gap-3 pb-2 border-b border-[#D6D0C4]">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-[#57534E] text-xs">Latency: <strong className="text-[#1C1917]">{req.time}</strong></span>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                        <div className="bg-[#EAE5DC] p-3 rounded-xl border border-[#D6D0C4] space-y-1">
                          <span className="text-[#78716C] uppercase font-semibold block text-[10px]">Request Payload / Headers:</span>
                          {req.headers && <p className="text-cyan-400 mb-1">{req.headers}</p>}
                          <p className="text-[#44403C] font-mono overflow-x-auto">{req.payload}</p>
                        </div>
                        <div className="bg-[#EAE5DC] p-3 rounded-xl border border-[#D6D0C4] space-y-1">
                          <span className="text-[#78716C] uppercase font-semibold block text-[10px]">Response Telemetry:</span>
          ...
          {activeTab === 'explorer' && (
            <div className="space-y-6 animate-fadeIn">
              <div className="bg-[#F7F5F0] border border-yellow-500/40 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-[#D6D0C4]">
                  <div>
                    <h3 className="text-xl font-extrabold text-[#1C1917] flex items-center gap-2.5">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left font-mono text-sm">
                    <thead>
                      <tr className="border-b border-[#D6D0C4] text-[#57534E] text-xs uppercase">
                    ...
                    <tbody className="divide-y divide-[#D6D0C4]">
                      {mockBlocks.map((blk) => (
                        <tr key={blk.height} className="hover:bg-[#EAE5DC] transition-colors">
                          ...
                          <td className="py-4 px-4 text-[#1C1917] font-semibold">{blk.vault}</td>
                          ...
                          <td className="py-4 px-4 text-[#57534E] text-xs">{blk.time}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
