...
export async function executeX402VerificationWorkflow({
  ...
  try {
    ...
    if (apiData.verified) {
      ...
      const defectsStr = apiData.defects && apiData.defects.length > 0 
        ? ` Detected defects: ${apiData.defects.join(', ')}.` 
        : '';
      log(5, `[REJECTED ❌] AI Oracle Confidence: ${((apiData.confidenceScore || 0.94) * 100).toFixed(1)}%. ${apiData.message}${defectsStr}`, 'error');
