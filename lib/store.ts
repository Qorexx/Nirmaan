...
import { WorkflowStage, TerminalLog, Project, ExternalService, StageId } from '@/lib/types';
...
const INITIAL_STAGES: WorkflowStage[] = [
  ...
  { id: 'gps-validation', stageNumber: 4, name: 'Satellite/GPS Validation', icon: '🛰️', layer: 'verify', status: 'pending', subtitle: 'Geo-Sync', details: ['Satellite Radar Cross-Check', 'Boundary Confirmed'], kpiMetric: '🛰️ Matched' },
...
const MOCK_SERVICES: ExternalService[] = [
  { name: 'Sovereign L1 Validator Node #402', status: 'Operational', pingMs: 12, type: 'Blockchain', endpoint: 'https://rpc.infra.gov.in/v1' },
  { name: 'Spatial AI GIS Engine', status: 'Operational', pingMs: 42, type: 'AI', endpoint: 'https://ai.morth.gov.in/infer' },
  { name: 'L402 Macaroon Gas Gateway (USDC)', status: 'Operational', pingMs: 18, type: 'Payment', endpoint: 'https://x402.gateway.org/pay' },
  { name: 'Satellite Radar Sync Module', status: 'Operational', pingMs: 104, type: 'Satellite', endpoint: 'https://risat.isro.gov.in/sync' },
];
...
export const useEscrowStore = create<EscrowState>((set, get) => ({
  ...
  loginUser: (newUser: Partial<AuthUser>) =>
    set((state) => ({
  ...
  logs: [
    {
      ...
      message: 'Autonomous Escrow Daemon initialized on Sovereign L1 Consensus (#402). Status: OPTIMIZED ($0.05 USDC).',
      type: 'info',
    },
    {
      id: 'init-2',
      ...
      stage: 'ai-processing',
      message: 'Connected to Spatial AI Telemetry stream. LiDAR + Infrared + EXIF defense active.',
      type: 'success',
      confidence: 99.4,
    },
  ],
      ...
      logs: [
        {
          ...
          timestamp: dayjs().format('HH:mm:ss.SSS'),
          stage: 'proof-submission',
          message: 'Simulation reset to idle operational baseline. Ready for new demonstration trigger.',
          type: 'info',
        },
      ],
    }),
  ...
  runLiveSimulation: (customProject) => {
    ...
    setTimeout(() => advanceStage(3, 'Geospatial validation: EXIF geotag matching against satellite radar boundaries.', 'info'), 2400);
  ...
  toggleFreezeProject: (projectId) => {
    set((state) => ({
      projects: state.projects.map((p) =>
        p.id === projectId ? { ...p, status: p.status === 'LOCKED' ? 'UNLOCKED' : 'LOCKED' } : p
      ),
    }));
  },
  ...
  overrideDecision: (projectId, manualApprove) => {
    set((state) => ({
      ...
      projects: state.projects.map((p) =>
        p.id === projectId ? { ...p, status: manualApprove ? 'UNLOCKED' : 'LOCKED' } : p
      ),
    }));
  },
}));
