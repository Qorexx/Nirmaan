...
const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-20 overflow-hidden text-[var(--color-text-primary)]">
      ...
      <section className="relative pt-12 sm:pt-16 pb-12 px-4 sm:px-6 max-w-[1700px] mx-auto flex flex-col items-center text-center">
        ...
        {/* Top Tagline Badge */}
        <div className="inline-flex items-center space-x-2.5 bg-surface border border-[var(--color-border-subtle)]/90 px-4 py-1.5 rounded-full text-xs font-mono font-extrabold text-indigo-700 mb-6 shadow-sm hover:shadow-md hover:scale-105 transition-all">
          <Award className="w-4 h-4 text-amber-500" />
          <span className="tracking-tight">National Infrastructure Governance SOVEREIGN EXECUTIVE EDITION</span>
          <span className="w-2 h-2 rounded-full bg-indigo-600 animate-pulse" />
        </div>

        {/* Main Title */}
        <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black text-[var(--color-text-primary)] font-heading tracking-tight max-w-5xl leading-[1.08]">
          The <span className="text-gradient-cyan">Autonomous Infrastructure</span> Operating System
        </h1>

        {/* Subtitle */}
        <p className="text-base sm:text-xl text-[var(--color-text-secondary)] font-heading font-normal max-w-3xl mt-6 leading-relaxed">
        ...
        {/* High-Impact Gov Stat Cascade (Apple Cupertino Grid) */}
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 w-full max-w-6xl mt-6">
          <div className="glass-card p-6 rounded-3xl border border-[var(--color-border-subtle)]/90 bg-surface text-center hover:scale-105 transition-transform shadow-xs">
          ...
          <div className="col-span-2 lg:col-span-1 glass-card p-6 rounded-3xl border border-[var(--color-border-subtle)]/90 bg-surface text-center hover:scale-105 transition-transform shadow-xs">
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-5">
          ...
          <button
            ...
            className="px-8 py-4 rounded-2xl bg-surface border border-[var(--color-border-subtle)]/90 hover:border-indigo-300 text-[var(--color-text-primary)] font-extrabold font-heading text-base transition-all hover:bg-[var(--color-bg-surface-secondary)] flex items-center gap-2.5 shadow-sm hover:shadow-md"
          >
      ...
      <section className="py-10 border-t border-b border-[var(--color-border-subtle)]/80 bg-[var(--color-bg-surface-secondary)]/70">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-xs font-mono uppercase tracking-widest text-indigo-600 font-black mb-2 flex items-center justify-center gap-1.5">
              ...
              <span>THE Enterprise WINNING WOW-FEATURE</span>
            </h2>
            <h3 className="text-3xl sm:text-4xl font-black font-heading text-[var(--color-text-primary)] tracking-tight">
              Interactive Digital Twin Inspection Suite
            </h3>
            <p className="text-sm sm:text-base text-[var(--color-text-secondary)] font-sans mt-2 max-w-2xl mx-auto">
          ...
          <div className="p-2 rounded-[32px] bg-surface border border-[var(--color-border-subtle)] shadow-xl">
            <BeforeAfterSlider 
      ...
      <section className="py-16 px-4 sm:px-6 max-w-[1700px] mx-auto border-t border-[var(--color-border-subtle)] bg-[var(--color-bg-surface-secondary)]/50">
        <div className="max-w-6xl mx-auto">
          <div className="mb-10 text-center sm:text-left">
            ...
            <h3 className="text-2xl sm:text-3xl font-black font-heading text-[var(--color-text-primary)] mt-1">Select an application module to begin demonstration</h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { id: '/command', title: '🏛️ Government Command', desc: 'Create projects, approve budgets, monitor vaults, override AI decisions, and set sovereign policies.', badge: 'Oversight', color: 'border-[var(--color-border-subtle)] text-indigo-600 hover:border-indigo-400' },
              ...
              { id: '/contractors', title: '🏗️ Contractor Workspace', desc: 'Software portal for submitting images, video streams, GPS coordinates, and inspection reports.', badge: 'Uploads', color: 'border-emerald-200 text-emerald-600 hover:border-emerald-400' },
              { id: '/ledger', title: '⛓️ Trust & x402 Layer', desc: 'Inspect zero-latency machine-to-machine HTTP 402 micropayments and immutable on-chain tx receipts.', badge: 'Ledger', color: 'border-amber-200 text-amber-600 hover:border-amber-400' },
              ...
              { id: '/audits', title: '📜 Audit & Compliance', desc: 'Tamper-proof regulatory dossiers, anti-corruption verification logs, and national executive archives.', badge: 'Registry', color: 'border-blue-200 text-blue-600 hover:border-blue-400' },
            ].map((app, idx) => (
              <div 
                key={idx}
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-black px-2.5 py-1 rounded-full bg-[var(--color-bg-surface-secondary)] border border-[var(--color-border-subtle)] text-[var(--color-text-primary)]">{app.badge}</span>
                  </div>
                  <h4 className="font-black font-heading text-lg text-[var(--color-text-primary)] mb-2 group-hover:text-indigo-600 transition-colors">{app.title}</h4>
                  <p className="text-xs text-secondary font-mono leading-relaxed">{app.desc}</p>
                </div>
                <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-heading font-extrabold text-[var(--color-text-secondary)] group-hover:text-indigo-600 transition-colors">
